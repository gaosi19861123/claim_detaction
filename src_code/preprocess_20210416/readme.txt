...
├── main.py: メイン関数
└── submodules: main.pyから呼び出されるサブモジュール群
...
- n_data_each_crash(int)
    - 1事故あたりの時系列データ数
    - サンプリングを複数回実施する場合は最後のサンプリングの後にだけ考慮される
- lim_d{x,y,z}_90_10(float)
    各事故における{x,y,z}軸加速度の「90%分位-10%分位」の上限（値の単位は入力ファイルと同一）

### sampling
- types(list of str)
    サンプリングの種類。リストに複数要素入れた場合はその順に実施。
    - "skip": サンプリングを行わない
    - "down": ダウンサンプリング（入力データの間引き）実行
    - "interp": 線形補間によるアップ・ダウンサンプリング実行
- interval_secs(list of float)
    出力する間隔秒数（サンプリングを実行する場合のみ考慮される）。リストの順番はtypesに対応させる。
- reference_time(str)
...
### computation
- factor_cpu_count(float, 0~1)
    - 欠損処理、サンプリングでの並列計算におけるCPU使用割合