...
def median_shift(df_ts, dict_config):
    ...
    print()
    print("【各事故各軸の加速度中央値計算後,時系列データをIDでgroupbyしたデータフレームの冒頭】")
    print(df_sum_med.head(), "\n")
    ...
    if "acceleration_z" in dict_config["input"]["time_series"]["column_types"]:
        dict_sum_median["med_acc_z"]=df_sum_med[("acceleration_x","median")]
    df_sum_median = pd.DataFrame(dict_sum_median)

    print("【時系列データに追加する中央値データフレーム冒頭】")
    print(df_sum_median.head(),"\n")
    ...
    del df_ts
    print("\n【中央値列追加後データフレーム冒頭】")
    print(df_med_added.head(), "\n")

    # 各軸加速度の中央値を0とするシフト補正実行して上書き
    if "acceleration_x" in dict_config["input"]["time_series"]["column_types"]:
        print("x軸加速度の中央値補正値計算中")
        df_med_added["acceleration_x"] \
            = df_med_added.apply(lambda x: \
                x["acceleration_x"]-x["med_acc_x"], axis=1)

    if "acceleration_y" in dict_config["input"]["time_series"]["column_types"]:
        print("y軸加速度の中央値補正値計算中")
        df_med_added["acceleration_y"] \
            = df_med_added.apply(lambda x: \
                x["acceleration_y"]-x["med_acc_y"], axis=1)

    if "acceleration_z" in dict_config["input"]["time_series"]["column_types"]:
        print("z軸加速度の中央値補正値計算中")
        df_med_added["acceleration_z"] \
            = df_med_added.apply(lambda x: \
                x["acceleration_z"]-x["med_acc_z"], axis=1)

    # 中央値列はreturnしない
    print("加速度中央値列削除中")
    df_mshifted = df_med_added[cols_df_ts]

    print("中央値補正後レコード数：", len(df_mshifted))
...
def normalize_with_rscaler(df_ts_train, df_ts_test, dict_config):
    ...
    print("各軸全事故一括のsk-learn robustscalerによる標準化")
    ...
    print("標準化後学習用データレコード数：", len(df_norm_train))
    print("標準化後検証用データレコード数：", len(df_norm_test))
...
def regularize(df_ts_train, df_ts_test, dict_config):
    ...
    if dict_config["regularization"]["type"]=="rscaler":
        ...
        print("config.jsonのregularization/typeの値が正しく設定されていません")
        sys.exit()
