...
class MSResNet(nn.Module):
    def __init__(self, input_channel, layers=[1, 1, 1, 1], num_classes=10):
        ...
        self.maxpool5 = nn.AvgPool1d(kernel_size=19, stride=1, padding=0)
    ...
    def forward(self, x0):
        ...
        # x = self.layer3x3_4(x)
        # x = self.maxpool3(x)
        x = F.avg_pool1d(x, kernel_size=x.size()[2], stride=1, padding=0)
        ...
        y = self.layer5x5_3(y)
        # y = self.layer5x5_4(y)
        #print("before_max_pooling_y_size:{}".format(y.size()))
        y = F.avg_pool1d(y, kernel_size=y.size()[2], stride=1, padding=0)
        #print("y_size:{}".format(y.size()))

        z = self.layer7x7_1(x0)
        z = self.layer7x7_2(z)
        z = self.layer7x7_3(z)
        # z = self.layer7x7_4(z)
        z = F.avg_pool1d(z, kernel_size=z.size()[2], stride=1, padding=0)
        #print("z_size:{}".format(z.size()))

        out = torch.cat([x, y, z], dim=1)

        out = out.squeeze()
        #print("output_size:{}".format(out.size()))
        # out = self.drop(out)
        out1 = self.fc(out)
